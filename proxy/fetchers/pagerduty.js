...
async function fetchPagerdutySatus(service) {
  ...
  const [servicesRes, postsRes] = await Promise.all([
    fetch(`${service.statusPageUrl}/api/services`),
    fetch(`${service.statusPageUrl}/api/posts?${activeStatuses}&limit=500`),
  ]);
  ...
  for (const p of activePosts) {
    const inc = new Incident({
      id:        p.id,
      name:      p.title ?? p.id,
      url: `${service.statusPageUrl}/posts/${p.id}`,
