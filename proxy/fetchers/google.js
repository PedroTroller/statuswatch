...
async function fetchGoogleIncidentDashboard(service) {
  const [incidentsRes, productsRes] = await Promise.all([
    fetch(`${service.statusPageUrl}/incidents.json`),
    fetch(`${service.statusPageUrl}/products.json`),
  ]);
  ...
  for (const i of active) {
    const inc = new Incident({
      ...
      url: `${service.statusPageUrl}/${i.uri}`,
  ...
  const components = allProducts.map(p => new ComponentStatus({
    id:              p.id,
    name:            p.title,
