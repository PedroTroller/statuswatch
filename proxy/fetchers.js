...
// Returns the more severe of two indicator strings.
function maxIndicator(a, b) {
  return (INDICATOR_RANK[a] ?? 0) >= (INDICATOR_RANK[b] ?? 0) ? a : b;
}
...
async function fetchStatuspageStatus(service) {
  ...
  const worstComponent = components.reduce(
    (worst, c) => maxIndicator(worst, componentStatusToIndicator(c.status) ?? 'none'),
    'none',
  );
  const indicator = maxIndicator(statusData.status.indicator, worstComponent);

  return makeResult(indicator, statusData.status.description, components, activeIncidents);
}
