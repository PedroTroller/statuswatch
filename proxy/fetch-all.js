...
const CATALOG    = Object.entries(new Function(`${catalogSrc}; return CATALOG;`)())
  .map(([id, s]) => ({ id, ...s }));
const ID_WIDTH   = Math.max(...CATALOG.map(s => s.id.length));

const { fetchServiceStatus } = require('./fetchers');

// ─── Terminal colors (no-op when not a TTY) ───────────────────────────────────
const C = process.stdout.isTTY
  ? { green: '\x1b[32m', red: '\x1b[31m', blue: '\x1b[96m', reset: '\x1b[0m' }
...
// background.js reads: status (string), description, components (plain objects),
...
async function main() {
  ...
  // Processed one service at a time. Status and icon results are printed inline.
  ...
  for (const service of CATALOG) {
    ...
    if (isFresh(cached)) {
      ...
      statusSkipped = false;
      try {
        result = toDistResult(await fetchWithRetry(service));
      } catch (err) {
        result = {
          status:          null,
          description:     '',
    ...
    // Icon
    const iconFile      = path.join(iconsDir, `${service.id}.png`);
    const iconWasCached = fs.existsSync(iconFile);
    let iconUrl;
    try {
      iconUrl = await fetchIconIfNeeded(service, iconsDir);
    } catch {
      iconUrl = null;
    }

    const statusMark = statusSkipped
      ? `${C.blue}skipped${C.reset}`
      : result.error ? `${C.red}failed ${C.reset}` : `${C.green}fetched${C.reset}`;
    const iconMark = iconWasCached
      ? `${C.blue}skipped${C.reset}`
      : iconUrl ? `${C.green}fetched${C.reset}` : `${C.red}failed ${C.reset}`;
    console.log(`  ${service.id.padEnd(ID_WIDTH)}  status:${statusMark}  icon:${iconMark}`);

    outcomes.push({ service, result, skipped: statusSkipped });
    iconOutcomes.push({ id: service.id, iconUrl });
  }
