...
// fetchFromCache is loaded from common/cache-reader.js.
...
// Returns an icon key: 'ok' | 'minor' | 'major' | 'critical' | 'maintenance' | 'default' (no data yet).
function worstStatus(states) {
  const statuses = Object.values(states).map(s => s?.status).filter(Boolean);
  if (!statuses.length)                                        return 'default';
  if (statuses.some(s => s === 'major_outage'))                return 'critical';
  if (statuses.some(s => s === 'partial_outage'))              return 'major';
  if (statuses.some(s => s === 'degraded_performance'))        return 'minor';
  if (statuses.some(s => s === 'under_maintenance'))           return 'maintenance';
  return 'ok';
}
...
function countDegraded(states) {
  return Object.values(states).filter(s => s?.status && s.status !== 'operational').length;
}
...
function findUntrackedCatalogEntry(url, services, catalog) {
  try {
    ...
    return catalog.find(entry => {
      ...
      if (new URL(entry.statusPageUrl).host === host) return true;
...
function notify(service, prevStatus, newStatus, description) {
  if (!service.notificationsEnabled) return;

  const wasOk         = !prevStatus || prevStatus === 'operational';
  const isMaintenance = newStatus === 'under_maintenance';
  const isOk          = newStatus === 'operational';
  ...
  if (!isOk && (STATUS_RANK[newStatus] ?? 0) < (STATUS_RANK[threshold] ?? 0)) return;
...
async function pollService(serviceId) {
  ...
  const _prevRaw = states[serviceId] ?? null;
  // Normalize states stored before the indicator→status migration.
  const INDICATOR_TO_STATUS = {
    none: 'operational', maintenance: 'under_maintenance', minor: 'degraded_performance',
    major: 'partial_outage', critical: 'major_outage',
  };
  const prev = (_prevRaw && _prevRaw.status === undefined && _prevRaw.indicator !== undefined)
    ? { ..._prevRaw, status: INDICATOR_TO_STATUS[_prevRaw.indicator] ?? 'operational' }
    : _prevRaw;
  ...
  let freshData;
  try {
    if (DEBUG) console.log(`[poll] fetching ${serviceId} (${service.name})`);
    freshData = await fetchFromCache(service);
    if (DEBUG) console.log(`[poll] ${serviceId} → ${freshData.status} | components: ${freshData.components?.length ?? 0} | incidents: ${freshData.activeIncidents?.length ?? 0} | generatedAt: ${freshData.generatedAt ?? 'n/a'}`);
    ...
    if (service.componentId) {
      ...
      if (comp) {
        if (DEBUG) console.log(`[poll] ${serviceId} → narrowed to component "${comp.name}" (${comp.id}): ${comp.status}`);
        freshData.status      = comp.status;
        freshData.description = COMP_STATUS_LABELS[comp.status] ?? comp.status ?? '';
        freshData.components  = [comp];
      } else {
        if (DEBUG) console.log(`[poll] ${serviceId} → component "${service.componentId}" not found in response`);
      }
    }
  } catch (err) {
    if (DEBUG) console.log(`[poll] ${serviceId} → error: ${err.message}`, err);
  ...
  if (prev && prev.status !== freshData.status && !freshData.error) {
    if (DEBUG) console.log(`[poll] ${serviceId} → status changed: ${prev.status} → ${freshData.status}`);
    notify(service, prev.status, freshData.status, freshData.description);
  }
...
// Only statusPageUrl is kept in sync (the display link may change); fetching is done
...
const CATALOG_SYNC_FIELDS = ['statusPageUrl'];
...
// Runs on every service worker start (including MV3 restarts triggered by alarms
// or messages). Immediately polls any service whose next fetch is already due so
// that fresh data is available without waiting for the alarm to fire.
(async () => {
  const { services, states } = await load();
  const now = Date.now();
  for (const service of services) {
    const st = states[service.id];
    if (!st || !st.nextFetchAt || now >= st.nextFetchAt) {
      pollService(service.id);
    }
  }
})();
