...
function renderCard(service, st, trackedCompIds = new Set()) {
  ...
  const dotCls    = serviceIndicatorDot(st?.indicator, st?.components);
      ...
      <div class="service-body">
        ...
        <div class="service-footer">
          ...
          <select class="threshold-select" data-action="threshold" data-id="${esc(service.id)}"
            ...
            <option value="maintenance" ${threshold === 'maintenance' ? 'selected' : ''}>≥ maint</option>
            <option value="minor"       ${threshold === 'minor'       ? 'selected' : ''}>≥ minor</option>
            <option value="major"       ${threshold === 'major'       ? 'selected' : ''}>≥ major</option>
            <option value="critical"    ${threshold === 'critical'    ? 'selected' : ''}>≥ crit</option>
          </select>` : ''}
    ...
    return `
      <div class="catalog-item" data-id="${esc(c.id)}">
        <div class="catalog-item-header">
          ${logoImg(c, 20)}
          ${c.indicator !== undefined ? statusIconSvg(serviceIndicatorDot(c.indicator, c.components)) : ''}
...
// Like indicatorDot but also handles the legacy case where indicator is 'none'
// but all components are under_maintenance (old cached files before maintenance
// became a first-class indicator).
function serviceIndicatorDot(indicator, components) {
  if (indicator === 'none' && components?.length > 0 &&
      components.every(c => c.status === 'under_maintenance')) {
    return 'maintenance';
  }
  return indicatorDot(indicator);
}
