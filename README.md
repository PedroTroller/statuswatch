...
`https://pedrotroller.github.io/statuswatch/` is generated by `site/build.js` at the
end of every "Fetch & publish status cache" run, from the `proxy/dist/catalog.json`
that run has just written. It groups all services by `category`, expands only the
ones that are not operational, and ships no JavaScript.

```bash
node proxy/fetch-all.js   # writes proxy/dist/catalog.json
node site/build.js        # renders proxy/dist/index.html from it
```

The generator reads no network, so it can be re-run locally against an existing
`proxy/dist/` while iterating on the layout.

`proxy/dist/` is gitignored: the page is a deployment artifact, not a committed one.

### Freshness monitoring

`proxy/check-freshness.js` fails when the published `catalog.json` stops being
refreshed, and `.github/workflows/freshness.yaml` runs it every 30 minutes.

It deliberately shares no concurrency group with the publishing workflow. When that
workflow stalled for four weeks, a run stuck on the `github-pages` environment gate
held the `pages` group and every later dispatch was cancelled while still pending,
so no step inside it ran and nothing inside it could report the problem.

An HTTP check would not have caught it either: `catalog.json` answered 200
throughout, serving data frozen on 6 August. `generatedAt` is the signal.

```bash
node proxy/check-freshness.js [url] [maxAgeMinutes]
```
...
| `category` | ✓ | One of `CATEGORIES` in `proxy/catalog.js`. Groups the service on the status page and in the list above. The validator rejects anything outside that set. |
...
├── site/
│   └── build.js         Renders proxy/dist/index.html, the published status page
├── cron-worker/
