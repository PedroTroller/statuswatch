...
const { Status }    = require('../common/value-objects/status.js');
...
function assertResult(service, result) {
  if (!(result instanceof Service))
    throw new Error('result must be a Service instance');
  if (typeof result.description !== 'string')
    throw new Error('description must be a string');
  for (const c of result.components) {
    if (!(c instanceof Component)) throw new Error(`Component is not a Component instance: ${JSON.stringify(c)}`);
  }
...
async function runAll() {
  ...
  const statusValues = Object.values(Status).map(s => s.value === 'operational' ? 'ok' : s.value);
