...
test('fetchIncidentioStatus: resolved incidents are filtered out', async () => {
  ...
  assert.equal(result.activeIncidents[0].url, 'https://iio.example.com/incidents/i2');
});
...
test('fetchUptimeRobotStatus: all down → critical', async () => {
  ...
  const result = await fetchUptimeRobotStatus(UPTIMEROBOT_SERVICE);
  assert.strictEqual(result.status, StatusEnum.MAJOR_OUTAGE); // all down → synthetic major_outage component
});
...
test('fetchStripeStatus: down → critical', async () => {
  ...
  const result = await fetchStripeStatus(STRIPE_SERVICE);
  assert.strictEqual(result.status, StatusEnum.MAJOR_OUTAGE); // no uptimeData → synthetic major_outage component
});
...
test('fetchAwsHealthStatus: monitoring events (status 3) → minor', async () => {
  ...
  const result = await fetchAwsHealthStatus(AWS_SERVICE);
  assert.strictEqual(result.status, StatusEnum.PARTIAL_OUTAGE); // component is partial_outage
  assert.equal(result.components.length, 1);
  assert.equal(result.components[0].status, 'partial_outage');
});
...
test('fetchAwsHealthStatus: investigating event (status 1) → major', async () => {
  ...
  const result = await fetchAwsHealthStatus(AWS_SERVICE);
  assert.strictEqual(result.status, StatusEnum.PARTIAL_OUTAGE); // all components are partial_outage
});
...
test('fetchAwsHealthStatus: deduplicates components by service+region', async () => {
  ...
  assert.equal(result.activeIncidents.length, 1);  // deduplicated by url (both point to same statusPageUrl)
});

// ─── ComponentStatus ────────────────────────────────────────────────────────────────

test('ComponentStatus: constructs with all valid statuses', () => {
  for (const status of COMPONENT_STATUSES) {
    const c = new ComponentStatus({ id: 'c1', name: 'API', status });
    assert.equal(c.status, status);
  }
});

const INC1 = new Incident({ id: 'i1', name: 'Outage',   url: 'https://example.com/i/i1', impact: 'major' });
const INC2 = new Incident({ id: 'i2', name: 'Degraded', url: 'https://example.com/i/i2', impact: 'minor' });

test('ComponentStatus: exposes all fields', () => {
  const c = new ComponentStatus({ id: 'c1', name: 'API', status: 'operational', activeIncidents: [INC1] });
...
test('ComponentStatus: activeIncidents defaults to empty array', () => {
  const c = new ComponentStatus({ id: 'c1', name: 'API', status: 'operational' });
  assert.deepEqual(Array.from(c.activeIncidents), []);
});

test('ComponentStatus: activeIncidents is a frozen immutable copy', () => {
  const incidents = [INC1];
  const c = new ComponentStatus({ id: 'c1', name: 'API', status: 'operational', activeIncidents: incidents });
...
test('ComponentStatus: rejects non-array activeIncidents', () => {
  assert.throws(
    () => new ComponentStatus({ id: 'c1', name: 'API', status: 'operational', activeIncidents: 'bad' }),
    TypeError,
  );
});

test('ComponentStatus: rejects non-Incident items in activeIncidents', () => {
  assert.throws(
    () => new ComponentStatus({ id: 'c1', name: 'API', status: 'operational', activeIncidents: [{ id: 'i1', name: 'x', url: 'https://x.com' }] }),
    TypeError,
  );
});

test('ComponentStatus: is immutable', () => {
  const c = new ComponentStatus({ id: 'c1', name: 'API', status: 'operational' });
  assert.throws(() => { c.status = 'major_outage'; }, TypeError);
  assert.throws(() => { c.id    = 'changed';       }, TypeError);
});

test('ComponentStatus: serializes to plain JSON', () => {
  const c    = new ComponentStatus({ id: 'c1', name: 'API', status: 'degraded_performance' });
  const json = JSON.parse(JSON.stringify(c));
  assert.deepEqual(json, { id: 'c1', name: 'API', status: 'degraded_performance', activeIncidents: [] });
});

test('ComponentStatus: rejects empty id', () => {
  assert.throws(
    () => new ComponentStatus({ id: '', name: 'API', status: 'operational' }),
    TypeError,
  );
});

test('ComponentStatus: rejects non-string id', () => {
  assert.throws(
    () => new ComponentStatus({ id: 42, name: 'API', status: 'operational' }),
    TypeError,
  );
});

test('ComponentStatus: rejects empty name', () => {
  assert.throws(
    () => new ComponentStatus({ id: 'c1', name: '', status: 'operational' }),
    TypeError,
  );
});


test('ComponentStatus: rejects unknown status', () => {
  assert.throws(
    () => new ComponentStatus({ id: 'c1', name: 'API', status: 'bad_status' }),
    TypeError,
  );
});

test('ComponentStatus: rejects undefined status', () => {
  assert.throws(
    () => new ComponentStatus({ id: 'c1', name: 'API', status: undefined }),
    TypeError,
  );
});
...
function makeComp(status, id = 'c1') {
  return new ComponentStatus({ id, name: 'API', status });
}
...
test('ServiceStatus: constructs with required fields and defaults', () => {
  const s = new ServiceStatus(BASE_SERVICE);
  assert.equal(s.id,          'github');
  assert.equal(s.name,        'GitHub');
  assert.equal(s.description, 'Code hosting');
  assert.equal(s.statusPageUrl,     'https://www.githubstatus.com');
  ...
  assert.strictEqual(s.status, StatusEnum.OPERATIONAL);
});

test('ServiceStatus: status is operational with no components', () => {
  const s = new ServiceStatus(BASE_SERVICE);
  assert.strictEqual(s.status, StatusEnum.OPERATIONAL);
});

test('ServiceStatus: status is the worst component status', () => {
  const s = new ServiceStatus({ ...BASE_SERVICE, components: [
  ...
  assert.strictEqual(s.status, StatusEnum.MAJOR_OUTAGE);
});

test('ServiceStatus: status respects severity order under_maintenance < degraded_performance < partial_outage < major_outage', () => {
  assert.strictEqual(new ServiceStatus({ ...BASE_SERVICE, components: [makeComp('under_maintenance')]    }).status, StatusEnum.UNDER_MAINTENANCE);
  assert.strictEqual(new ServiceStatus({ ...BASE_SERVICE, components: [makeComp('degraded_performance')] }).status, StatusEnum.DEGRADED_PERFORMANCE);
  assert.strictEqual(new ServiceStatus({ ...BASE_SERVICE, components: [makeComp('partial_outage')]       }).status, StatusEnum.PARTIAL_OUTAGE);
  assert.strictEqual(new ServiceStatus({ ...BASE_SERVICE, components: [makeComp('major_outage')]         }).status, StatusEnum.MAJOR_OUTAGE);
});

test('ServiceStatus: activeIncidents is the unique merge of all component activeIncidents', () => {
  const inc3 = new Incident({ id: 'i3', name: 'CDN', url: 'https://example.com/i/i3' });
  const c1 = new ComponentStatus({ id: 'c1', name: 'API', status: 'major_outage',         activeIncidents: [INC1, INC2] });
  const c2 = new ComponentStatus({ id: 'c2', name: 'Web', status: 'degraded_performance', activeIncidents: [INC2, inc3] });
  const c3 = new ComponentStatus({ id: 'c3', name: 'CDN', status: 'operational',          activeIncidents: [] });
  const s  = new ServiceStatus({ ...BASE_SERVICE, components: [c1, c2, c3] });
  // INC2 appears in both c1 and c2 but is deduplicated by url
  assert.deepEqual(Array.from(s.activeIncidents), [INC1, INC2, inc3]);
});

test('ServiceStatus: activeIncidents is empty when no components have incidents', () => {
  const s = new ServiceStatus({ ...BASE_SERVICE, components: [makeComp('operational')] });
  assert.deepEqual(Array.from(s.activeIncidents), []);
});

test('ServiceStatus: activeIncidents is frozen', () => {
  const s = new ServiceStatus(BASE_SERVICE);
  assert.throws(() => { s.activeIncidents.push({ id: 'i1' }); }, TypeError);
});

test('ServiceStatus: relatedDomains and searchAliases are frozen immutable copies', () => {
  const domains  = ['github.com'];
  const aliases  = ['git'];
  const s = new ServiceStatus({ ...BASE_SERVICE, relatedDomains: domains, searchAliases: aliases });
...
test('ServiceStatus: components is a frozen immutable copy', () => {
  const list = [makeComp('operational')];
  const s    = new ServiceStatus({ ...BASE_SERVICE, components: list });
...
test('ServiceStatus: serializes to plain JSON', () => {
  const comp = makeComp('degraded_performance');
  const s    = new ServiceStatus({
    ...BASE_SERVICE,
...
test('ServiceStatus: accepts null and valid ISO string for fetchedAt', () => {
  assert.doesNotThrow(() => new ServiceStatus({ ...BASE_SERVICE, fetchedAt: null }));
  assert.doesNotThrow(() => new ServiceStatus({ ...BASE_SERVICE, fetchedAt: '2026-04-10T16:00:00.000Z' }));
  assert.doesNotThrow(() => new ServiceStatus({ ...BASE_SERVICE, fetchedAt: new Date().toISOString() }));
});

test('ServiceStatus: rejects empty id', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, id: '' }),   TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, id: 42  }),  TypeError);
});

test('ServiceStatus: rejects empty name', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, name: ''  }), TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, name: null }), TypeError);
});

test('ServiceStatus: rejects non-string description', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, description: undefined }), TypeError);
});

test('ServiceStatus: rejects invalid statusPageUrl', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, statusPageUrl: 'not-a-url' }), TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, statusPageUrl: ''           }), TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, statusPageUrl: null         }), TypeError);
});

test('ServiceStatus: rejects non-array or non-string-item relatedDomains', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, relatedDomains: 'github.com' }), TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, relatedDomains: [42]         }), TypeError);
});

test('ServiceStatus: rejects non-array or non-string-item searchAliases', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, searchAliases: 'git' }), TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, searchAliases: [{}]  }), TypeError);
});

test('ServiceStatus: rejects invalid fetchedAt', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, fetchedAt: 'not-a-date'  }), TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, fetchedAt: 0             }), TypeError);
});

test('ServiceStatus: rejects non-boolean fetchedFailed', () => {
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, fetchedFailed: 1      }), TypeError);
  assert.throws(() => new ServiceStatus({ ...BASE_SERVICE, fetchedFailed: 'true' }), TypeError);
});

test('ServiceStatus: rejects non-Component items in components', () => {
  assert.throws(
    () => new ServiceStatus({ ...BASE_SERVICE, components: [{ id: 'c1', name: 'API', status: 'operational' }] }),
    TypeError,
  );
  assert.throws(
    () => new ServiceStatus({ ...BASE_SERVICE, components: ['okay'] }),
    TypeError,
  );
});
...
test('Incident: constructs with all fields', () => {
  const i = new Incident({ id: 'i1', name: 'Outage', url: 'https://example.com/i/i1', impact: 'major' });
  assert.equal(i.id,        'i1');
  assert.equal(i.name,      'Outage');
  assert.equal(i.url, 'https://example.com/i/i1');
  assert.equal(i.impact,    'major');
});

test('Incident: id and impact default to null', () => {
  const i = new Incident({ name: 'Outage', url: 'https://example.com/i/i1' });
...
test('Incident: rejects empty name', () => {
  assert.throws(() => new Incident({ name: '', url: 'https://example.com' }), TypeError);
});

test('Incident: rejects invalid url', () => {
  assert.throws(() => new Incident({ name: 'Outage', url: 'not-a-url' }), TypeError);
  assert.throws(() => new Incident({ name: 'Outage', url: ''           }), TypeError);
});

test('Incident: rejects empty string id', () => {
  assert.throws(() => new Incident({ id: '', name: 'Outage', url: 'https://example.com' }), TypeError);
});

test('Incident: rejects non-string impact', () => {
  assert.throws(() => new Incident({ name: 'Outage', url: 'https://example.com', impact: 42 }), TypeError);
});

// ─── Status ───────────────────────────────────────────────────────────────────

test('StatusEnum: has exactly five entries', () => {
  assert.equal(Object.keys(StatusEnum).length, 5);
});

test('StatusEnum: entries have the correct values', () => {
  assert.equal(StatusEnum.OPERATIONAL.value,          'operational');
  assert.equal(StatusEnum.UNDER_MAINTENANCE.value,    'under_maintenance');
  assert.equal(StatusEnum.DEGRADED_PERFORMANCE.value, 'degraded_performance');
  assert.equal(StatusEnum.PARTIAL_OUTAGE.value,       'partial_outage');
  assert.equal(StatusEnum.MAJOR_OUTAGE.value,         'major_outage');
});

test('StatusEnum: entries have the correct levels', () => {
  assert.equal(StatusEnum.OPERATIONAL.level,          0);
  assert.equal(StatusEnum.UNDER_MAINTENANCE.level,    1);
  assert.equal(StatusEnum.DEGRADED_PERFORMANCE.level, 2);
  assert.equal(StatusEnum.PARTIAL_OUTAGE.level,       3);
  assert.equal(StatusEnum.MAJOR_OUTAGE.level,         4);
});

test('StatusEnum: levels are strictly ordered operational < under_maintenance < degraded_performance < partial_outage < major_outage', () => {
  const entries = Object.values(StatusEnum).sort((a, b) => a.level - b.level);
...
test('StatusEnum: each entry is frozen', () => {
  for (const entry of Object.values(StatusEnum)) {
    assert.throws(() => { entry.level = 99; }, TypeError);
  }
});

test('StatusEnum: StatusEnum itself is frozen', () => {
  assert.throws(() => { StatusEnum.NEW = {}; }, TypeError);
});
