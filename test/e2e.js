...
const { fetchFromCache } = require('../common/cache-reader.js');
...
function assertCacheEntry(data) {
  if (typeof data.status !== 'string' || !VALID_STATUSES.has(data.status))
    throw new Error(`status must be a valid status string, got ${JSON.stringify(data.status)}`);
...
async function runAll() {
  ...
  for (const service of CATALOG) {
    ...
    try {
      ...
      if (data.error) {
        if (service.beta) {
          console.log(`${C.yellow}–${C.reset} ${label}  ${C.dim}[beta] fetch error: ${data.error}${C.reset}`);
          continue;
        }
        console.log(`${C.red}✗${C.reset} ${label}  ${C.red}fetch error: ${data.error}${C.reset}`);
        failed++;
        continue;
      }
      assertCacheEntry(data);
      const compCount = `${data.components.length} component${data.components.length !== 1 ? 's' : ''}`;
      console.log(`${C.green}✓${C.reset} ${label}  ${C.dim}${data.status.padEnd(20)}  ${compCount}${C.reset}`);
      passed++;
    } catch (err) {
